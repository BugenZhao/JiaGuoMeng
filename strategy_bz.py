...
residence = '木屋 居民楼 钢结构房 平房 人才公寓 小型公寓 中式小楼 花园洋房'.split()
...
OneStars = '企鹅机械 媒体之声 人民石油 中式小楼 花园洋房 加油站 商贸中心'.split()
TwoStars = '人才公寓 纺织厂 钢铁厂 图书城 民食斋 电厂 零件厂'.split()
TriStars = '木屋 服装店 食品厂 木材厂 学校 小型公寓 水厂'.split()
QuaStars = '五金店 居民楼 造纸厂 菜市场 平房 便利店 钢结构房 '.split()
...
for item in commercial:  # 商业
    start[item] = startDict[star[item]] * (8.5 + 0.2) * (1 + 1.2 + 0.6) * 1.2
for item in industry:  # 工业
    start[item] = startDict[star[item]] * (10 + 0.2) * (1 + 1.2 + 1.5) * 1.2
for item in residence:  # 住宅
    start[item] = startDict[star[item]] * (8.5 + 0.2) * (1 + 1.2 + 1.5) * 1.5
