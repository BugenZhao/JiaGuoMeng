...
residence = '木屋 居民楼 钢结构房 平房 人才公寓 小型公寓'.split()
...
OneStars = '零件厂 人才公寓 企鹅机械 媒体之声 小型公寓'.split()
TwoStars = '纺织厂 钢铁厂 木屋 图书城 民食斋 电厂'.split()
TriStars = '居民楼 服装店 食品厂 便利店 木材厂 平房 菜市场 钢结构房 五金店 造纸厂 学校'.split()
...
for item in commercial:  # 商业
    start[item] = startDict[star[item]] * (1 + 1 + 3 + 1 + 0.5 + 0.1) * (1 + 1.2 + 0.6)
for item in industry:  # 工业
    start[item] = startDict[star[item]] * (1 + 1 + 1 + 0.5 + 1.5 + 0.1) * (1 + 1.2 + 1.5)
for item in residence:  # 住宅
    start[item] = startDict[star[item]] * (1 + 1 + 3 + 1 + 0.5 + 0.1) * (1 + 1.2 + 1.5) * 1.3
